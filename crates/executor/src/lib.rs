...
pub trait DynProvider: Send + Sync {
    ...
    async fn list(&self, kind: guildforge_provider::ResourceKind) -> Result<Vec<Resource>, ProviderError>;
    /// Provider name.
...
impl Executor {
    ...
    pub async fn execute(
        ...
        // Map category names to their Discord IDs.
        let mut category_ids: std::collections::HashMap<String, String> = std::collections::HashMap::new();
        ...
        for op in &plan.operations {
            ...
            if failed_addrs.iter().any(|f| addr.as_str().starts_with(f.as_str())) {
                report.skipped += 1;
            ...
            // For channel Creates, inject parent_id from category map.
            let op_to_run = if let guildforge_planner::Operation::Create { desired } = op {
                if let guildforge_provider::Resource::Channel(ref ch) = desired {
                    let parts: Vec<&str> = ch.addr.as_str().splitn(3, '/').collect();
                    if parts.len() == 3 && parts[1] != "_top" {
                        let cat_name = parts[1];
                        if let Some(parent_id_str) = category_ids.get(cat_name) {
                            if let Ok(id) = parent_id_str.parse::<u64>() {
                                let mut new_ch = ch.clone();
                                new_ch.parent_id = Some(guildforge_shared::Snowflake::new(id));
                                guildforge_planner::Operation::Create {
                                    desired: guildforge_provider::Resource::Channel(new_ch),
                                }
                            } else {
                                op.clone()
                            }
            ...
            // Execute and get the created resource back.
            let created_resource = match self.execute_op_returning(&op_to_run, &mut report, tx).await {
                Ok(r) => r,
                Err(ExecutorError::Permanent { addr: fail_addr, .. }) => {
                    failed_addrs.push(fail_addr);
            ...
            // If a category was created, save its Discord ID.
            if let Some(resource) = &created_resource {
                if let guildforge_provider::Resource::Category(cat) = resource {
                    if let Some(id) = cat.id {
                        category_ids.insert(cat.name.clone(), id.to_string());
                    }
                }
            }

            debug!(%addr, "op complete");
        }
    ...
    /// Execute a single operation. Returns the created/updated resource
    /// (with Discord-assigned IDs) so the caller can use it.
    async fn execute_op_returning(
        &self,
        op: &guildforge_planner::Operation,
        report: &mut ExecutionReport,
        tx: &mut Transaction,
    ) -> Result<Option<guildforge_provider::Resource>, ExecutorError> {
        ...
        loop {
            let result = match op {
                ...
                Operation::Update { current, desired } => self.provider.update(current, desired).await,
    ...
    pub async fn read(&self, addr: &ResourceId) -> Result<Option<Resource>, ProviderError> {
        self.provider.read(addr).await
    }

    /// List all resources of a given kind.
    pub async fn list(&self, kind: guildforge_provider::ResourceKind) -> Result<Vec<Resource>, ProviderError> {
        self.provider.list(kind).await
    }
